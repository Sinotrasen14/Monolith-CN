# MARK: Medical

uplink-pirate-medical-bundle-name = 医疗包
uplink-pirate-medical-bundle-desc = 各种自动注射笔和高级医疗设备，可应对各种可能的情况。内含一支可作为武器使用的精英紧凑型除颤器。

uplink-pirate-interdyne-defibrillator-name = 高级除颤器
uplink-pirate-interdyne-defibrillator-desc = 一支精英紧凑型除颤器，可作为武器使用。出奇地致命。

uplink-pirate-chemical-synthesis-kit-name = 化学合成套件
uplink-pirate-chemical-synthesis-kit-desc = 为有抱负的化学家准备的入门套件，包含毒素和vestine，满足你的所有生物武器需求！

uplink-pirate-vestine-vial-name = 维斯廷·维亚尔 (30u)
uplink-pirate-vestine-vial-desc = 一小瓶昂贵的异常液体，由皮兹特本地动物合成。对高级化学生产很有用。

uplink-pirate-anti-poison-pen-name = 解毒自动注射笔
uplink-pirate-anti-poison-pen-desc = 快速剂量的解毒剂。含有ultravasculine和肾上腺素。

uplink-pirate-puncturase-pen-name = puncturase自动注射笔
uplink-pirate-puncturase-pen-desc = 快速剂量的puncturase和氨甲环酸，用于战斗场景。

uplink-pirate-pyrazine-pen-name = Pyrzine自动注射笔
uplink-pirate-pyrazine-pen-desc = 快速剂量的pyrazine和dermaline，用于战斗场景。

uplink-pirate-rad-pen-name = 抗辐射自动注射笔
uplink-pirate-rad-pen-desc = 快速剂量的抗辐射剂。含有arithrazine和bicaridine。

uplink-pirate-combat-medkit-name = 战斗医疗箱
uplink-pirate-combat-medkit-desc = 一个医疗箱，内含治疗创伤、挫伤和失血的物资。

uplink-pirate-combat-pen-name = 战斗医疗笔
uplink-pirate-combat-pen-desc = 一支通用omnizine注射剂，包装在自动注射笔中。

uplink-pirate-syringe-case-name = 医疗笔盒
uplink-pirate-syringe-case-desc = 一个便携盒，装满适用于大多数战斗场景的医疗笔。

uplink-pirate-hyperzine-name = hyperzine注射器
uplink-pirate-hyperzine-desc = 战斗兴奋剂，可防止眩晕并提升你的移动能力。持续30秒。

uplink-pirate-surgery-name = 手术套件
uplink-pirate-surgery-desc = 一个手术套件，包含挽救朋友或肢解敌人所需的最低限度工具。

uplink-pirate-omnizine-jug-name = omnizine大罐
uplink-pirate-omnizine-jug-desc = 一罐omnizine，可用于制作医疗笔并维持盟友的生命。从和谐医疗企业悄悄进口。

uplink-pirate-medical-assembler-name = 医疗组装机预制包
uplink-pirate-medical-assembler-desc = 一台可用于制作实用医疗物资的组装机。

# MARK: Utility

uplink-pirate-wearable-camerapdv-name = 无线摄像头
uplink-pirate-wearable-camerapdv-desc = 用于监视作战区域的相机。可以佩戴或放置在某个地方，配备变色龙技术，可以伪装成任何类型的项链饰品。

uplink-pirate-romerol-name = romerol注射器
uplink-pirate-romerol-desc = 一种在以往战争中使用的生物武器，已被严厉禁止。显然走私这东西花了不少功夫。能让死者复生。

uplink-pirate-empty-implanter-name = 空植入器
uplink-pirate-empty-implanter-desc = 一个备用的空植入器，用于移除植入物，包括阵营追踪植入物。它不会对你产生反噬。

uplink-pirate-syndicate-magboots-name = 战术磁力靴
uplink-pirate-syndicate-magboots-desc = 一双可防止滑倒的靴子，在零重力下可让你正常移动，代价是略微减速。此外，它具备喷气背包功能并自带燃料，但持续时间不长。

uplink-pirate-syndicate-jetpack-name = 黑色喷气背包
uplink-pirate-syndicate-jetpack-desc = 一个黑色喷气背包。可让你在太空中飞行。不含燃料补充，请明智使用燃料。

uplink-pirate-teleshield-name = 伸缩盾
uplink-pirate-teleshield-desc = 一款可伸缩的手持盾牌，提供出色的保护。

uplink-pirate-energy-shield-name = 能量护盾
uplink-pirate-energy-shield-desc = 奇异能量盾，可以反射几乎所有激光束，同时对子弹和其他物理攻击也有一点保护。

uplink-pirate-radio-jammer-name = 无线电干扰器
uplink-pirate-radio-jammer-desc = 该设备启动后会干扰附近所有发出的无线电通讯以及防护服传感器。

uplink-pirate-overwatch-jammer-name = 先锋监视干扰器
uplink-pirate-overwatch-jammer-desc = 这个装置可以防止守望主机检测到它附着的尸体。搬运尸体去太阳神号之前一定要先装上一个！

uplink-pirate-hypopen-name = 隐形注射笔
uplink-pirate-hypopen-desc = 一支伪装成钢笔的化学低压注射器，能瞬间注射最多10u的试剂。初始为空。

uplink-pirate-implanter-freelance-name = Vanguard植入器
uplink-pirate-implanter-freelance-desc = 植入一支Vanguard无线电，无需耳机即可进行隐蔽通讯。

uplink-pirate-emag-name = 密码序列器
uplink-pirate-emag-desc = 一种多用途黑客设备，可配置某些机器的工作方式。

uplink-pirate-access-breaker-name = 认证干扰器
uplink-pirate-access-breaker-desc = 一种可覆盖机器或门上读卡器的设备，让你可自由通行。

uplink-pirate-jaws-name = 高级液压剪
uplink-pirate-jaws-desc = 一把经过非法改装突破限制的普通液压剪，开门速度是普通JOL的2倍。

uplink-pirate-thruster-kit-name = 推进器升级套件
uplink-pirate-thruster-kit-desc = 内含32个超级电容。正好升级八个推进器。

uplink-pirate-comms-computer-circuitboard-name = PDV通讯计算机电路板
uplink-pirate-comms-computer-circuitboard-desc = 一块用于PDV通讯控制台的计算机印刷电路板。

uplink-pirate-security-cyborg-module-name = 安保赛博格模块
uplink-pirate-security-cyborg-module-desc = 一把弯刀、一把能量枪和一根眩晕警棍，专为赛博格设计。等等，TFSMC不是有一样的东西吗？

uplink-pirate-voicemask-name = 变色龙语音掩码
uplink-pirate-voicemask-desc = 一个可以用来搞点小伎俩和恶作剧的语音面具……或者只是为了隐藏你的身份。在先锋无线电上说话时掩盖你的声音是极力建议的。

uplink-pirate-blank-id-name = 空白ID卡
uplink-pirate-blank-id-desc = 一张空白的流浪者身份证，没有任何识别特征。可以在Helios的身份证控制台上随心定制。

uplink-pirate-chameleon-pda-name = 变色龙PDA
uplink-pirate-chameleon-pda-desc = 一款带有变色龙技术的PDA，让你可以隐藏自己的真实立场。

uplink-pirate-chameleon-headset-name = 变色龙耳机
uplink-pirate-chameleon-headset-desc = 一款内置变色龙技术的耳机，让你可以隐藏自己的真实立场。

uplink-pirate-chameleon-backpack-name = 变色龙背包
uplink-pirate-chameleon-backpack-desc = 一个带有变色龙技术的背包，让你可以隐藏自己的真实立场。不包括任何内部物品。

uplink-pirate-dna-scrambler-name = DNA 搅拌器植入器
uplink-pirate-dna-scrambler-desc = 一种相对罕见的植入物，可以让你一次性完全改变身份。

uplink-pirate-cash10000-name = 10,000信用点
uplink-pirate-cash10000-desc = 好小子，你让我今天心情大好。

uplink-pirate-cash100000-name = 100,000信用点
uplink-pirate-cash100000-desc = 好小子，你真是让我今天心情大好。
# desc is quote

uplink-syndicate-applicator-name = 高级纳米施加器
uplink-syndicate-applicator-desc = 高级纳米施加器，具有大幅升级的纳米容量，能够自我生成纳米体。

# MARK: Weapons

uplink-pirate-energy-cutlass-name = 能量弯刀
uplink-pirate-energy-cutlass-desc = 一种异域能量武器。

uplink-pirate-energy-sword-name = 能量剑
uplink-pirate-energy-sword-desc = 留下你的印记，不留活口。少数成功越狱破解的TSF能量武器之一。

uplink-pirate-python-name = Python
uplink-pirate-python-desc = 一把简单粗暴、有效且响亮的左轮手枪。预装.357 magnum弹药。

uplink-pirate-viper-name = Viper
uplink-pirate-viper-desc = 一把小巧、易于隐藏但威力略显不足的枪。改装了全自动机匣。使用手枪弹匣（9x19mm FMJ）。

uplink-pirate-kanabou-name = Kanabou
uplink-pirate-kanabou-desc = 给那些想放弃含蓄的人的武器。特别适合鬼族使用。

uplink-pirate-ak-name = AKM
uplink-pirate-ak-desc = 一把标志性的战争武器。使用7.62x39mm弹药。

uplink-pirate-laser-gun-name = 复古激光爆能枪
uplink-pirate-laser-gun-desc = 一把民用级武器，利用受激辐射放大光波。

uplink-pirate-c20-name = C-20r套装
uplink-pirate-c20-desc = 一把9x19mm轻型冲锋枪，附带几个弹匣。

uplink-pirate-vympel-name = 标杆
uplink-pirate-vympel-desc = 一把便宜的5.56x45毫米犊牛式步枪。

uplink-pirate-sultan-pulsar-name = 苏丹的脉冲星
uplink-pirate-sultan-pulsar-desc = 一把4号口径的猎枪，射击散布非常集中。

uplink-pirate-atreides-name = 亚特雷ides礼包
uplink-pirate-atreides-desc = 一把5.7x28毫米的单手轻型冲锋枪，配有一些弹匣。

uplink-pirate-bulldog-name = Bulldog套装
uplink-pirate-bulldog-desc = 一把自动12 gauge霰弹枪，附带几个弹鼓。

uplink-pirate-m90-name = M-90
uplink-pirate-m90-desc = 一款紧凑型犊牛式7.62x51毫米战斗步枪，设计上能以突击步枪的射速开火。

uplink-pirate-mla73-name = MLA-73
uplink-pirate-mla73-desc = 一款亚音速、精准的冲锋枪，使用6.35x40mm 无壳子弹，对装甲有效。紧急情况下也可以用9x19mm。

uplink-pirate-vt7-name = VT7高频刀
uplink-pirate-vt7-desc = 一把高品质高频刀。

uplink-pirate-desword-name = 双刃能量剑
uplink-pirate-desword-desc = 能量剑的双手变种。

uplink-pirate-anaconda-name = Anaconda
uplink-pirate-anaconda-desc = 一把手持式、紧凑型、可自充能的弹道武器。

uplink-pirate-wspr-name = WSPR
uplink-pirate-wspr-desc = 针对TSFMC的Annie的对策武器。使用相同的7.62x39mm弹药。

uplink-pirate-burner-name = Burner重型步枪
uplink-pirate-burner-desc = 一把破坏性的12.7x99mm步枪。支持HE弹药。破坏力惊人。

uplink-pirate-basilisk-name = Basilisk DEW
uplink-pirate-basilisk-desc = 一把逆向工程的辐射步枪。对电磁脉冲免疫，而且它的光束用肉眼看不见。

uplink-pirate-hristov-name = 赫里斯托夫套装
uplink-pirate-hristov-desc = 一把14.5x114毫米的反器材步枪，能发射毁灭性的子弹，彻底穿透人体，轻松击穿机甲、全息屏障、硬盾或气闸。

uplink-pirate-blueprint-launcher-rocket-name = RPG-7蓝图
uplink-pirate-blueprint-launcher-rocket-desc = RPG-7及其弹药的蓝图。适合长期恐怖活动！

uplink-pirate-grenade-launcher-bundle-name = China Lake套装
uplink-pirate-grenade-launcher-bundle-desc = 噗！

# MARK: Ammo

uplink-pirate-box-magazine-pistol-name = 一盒手枪9x19mm弹匣
uplink-pirate-box-magazine-pistol-desc = 一个装满4个9x19mm手枪弹匣的盒子。

uplink-pirate-box-magazine-light-rifle-name = 一盒7.62x39mm弹匣
uplink-pirate-box-magazine-light-rifle-desc = 一个装满4个7.62x39mm弹匣的盒子。

uplink-pirate-magazine-box-357_magnum-name = 一盒.357 magnum快装弹匣
uplink-pirate-magazine-box-357_magnum-desc = 一个装有4个.357 magnum快装弹匣的盒子。

uplink-pirate-light-rifle-magazine-box-big-name = 弹药箱（7.62x39mm FMJ）
uplink-pirate-light-rifle-magazine-box-big-desc = 一个装有200发7.62x39mm弹药的盒子。

uplink-pirate-shell-box-lethal-name = 12号径 霰弹盒（致命）
uplink-pirate-shell-box-lethal-desc = 一盒装有32发12号猎枪弹的子弹。

uplink-pirate-shell-box-4g-name = 4号径 霰弹盒（致命）
uplink-pirate-shell-box-4g-desc = 一盒装有32发4号猎枪弹的子弹。

uplink-pirate-smg-ammo-box-name = 弹药箱（9x19mm FMJ）
uplink-pirate-smg-ammo-box-desc = 一个装有60发9x19mm手枪弹药的盒子。可用于装填冲锋枪弹匣。

uplink-pirate-box-bulldog-ammo-name = 一盒12 gauge鹿弹弹鼓
uplink-pirate-box-bulldog-ammo-desc = 一个装有4个12 gauge鹿弹霰弹鼓的盒子。

uplink-pirate-box-bulldog-slug-ammo-name = 一盒12 gauge独头弹弹鼓
uplink-pirate-box-bulldog-slug-ammo-desc = 一个装有4个12 gauge独头弹霰弹鼓的盒子。

uplink-pirate-box-smg-magazine-name = 一盒9x19mm冲锋枪弹匣
uplink-pirate-box-smg-magazine-desc = 一个装满4个9x19mm冲锋枪弹匣的盒子。

uplink-pirate-box-5-7x28mm-smg-name = 一盒5.7x28mm冲锋枪弹匣
uplink-pirate-box-5-7x28mm-smg-desc = 一盒装有4个适用于阿特雷德斯的5.7x28毫米冲锋枪弹匣，对装甲非常有效。

uplink-pirate-7-62x51mmMagazine-name = 7.62x51mm 弹夹
uplink-pirate-7-62x51mmMagazine-desc = 一个装有4个填充了7.62x51mm子弹的弹匣的盒子。

uplink-pirate-6-35x40mm-magazine-name = 6.35x40mm 无壳弹匣
uplink-pirate-6-35x40mm-magazine-desc = MLA-73 的亚音速弹匣，对装甲有效。

uplink-pirate-10phz-magazine-name = 10Phz DEW 弹夹
uplink-pirate-10phz-magazine-desc = Basilisk用的放射性定向能武器弹匣。

uplink-subsonic-mag-name = 7.62x39mm亚音速步枪弹匣
uplink-subsonic-mag-desc = 支持WSPR。

uplink-subsonic-box-name = 7.62x39mm亚音速步枪弹药箱
uplink-subsonic-box-desc = 一盒WSPR的弹药。

uplink-pirate-box-highcal-name = 12.7x99mm弹药箱
uplink-pirate-box-highcal-desc = 一盒Burner重型步枪的通用弹药。

uplink-pirate-box-highcal-he-name = HE 12.7x99mm弹药箱
uplink-pirate-box-highcal-he-desc = 一盒Burner重型步枪的爆破弹药。

uplink-pirate-sniper-ammo-name = 14.5x114mm AP弹药箱
uplink-pirate-sniper-ammo-desc = 一盒Hristov用的大口径穿甲狙击弹药。

uplink-pirate-emp-rocket-name = RPG-7 EMP火箭弹
uplink-pirate-emp-rocket-desc = 一枚对一切电气设备怀有仇怨的火箭弹。在其范围内使电气物品瘫痪并耗尽其电量。

uplink-pirate-frag-grenade-name = 破片杀伤手榴弹
uplink-pirate-frag-grenade-desc = 一枚用于China Lake的40mm破片杀伤手榴弹，破坏范围极广。摧毁大片区域内的一切。

uplink-pirate-blast-grenade-name = 爆破手榴弹
uplink-pirate-blast-grenade-desc = 一枚用于China Lake的40mm温压手榴弹，杀伤力极强。在精确的小半径范围内具有极高破坏力。

# MARK: Equipment

uplink-pirate-combat-hardsuit-name = SCAF战术服
uplink-pirate-combat-hardsuit-desc = PDV军事组织的基础防护装备。

uplink-pirate-cv32-hardsuit-name = CV-32 战斗硬装
uplink-pirate-cv32-hardsuit-desc = 一套装备了出色防弹护盾的硬壳服，不过在其他方面相当欠缺。

uplink-pirate-cv53-tacsuit-name = CV-53 战斗硬装
uplink-pirate-cv53-tacsuit-desc = 一套配备了内置护盾发生器以及出色环境和激光防护的战术服。不过，护盾本身对激光很脆弱，而护甲对弹道武器也不太抗打。

uplink-pirate-cv82-tacsuit-name = CV-82 战斗硬装
uplink-pirate-cv82-tacsuit-desc = 一套灵活的战术服，配备了内置的动作预测功能，可以加快穿戴者的移动速度。

uplink-pirate-juggernaut-hardsuit-name = 巨兽重装甲
uplink-pirate-juggernaut-hardsuit-desc = 这套重装战甲的蓝图直接来源于苏丹本人，能够轻松抵挡各种攻击。装备非常重。

uplink-pirate-stealth-hardsuit-name = 隐身重装甲服
uplink-pirate-stealth-hardsuit-desc = 这套重装甲的设计图源自苏丹本人，它用装甲板换成能够自动融入周围环境的隐形板。


uplink-pirate-viper-group-standard-hardsuit-name = 毒蛇小队JACKAL硬甲服
uplink-pirate-viper-group-standard-hardsuit-desc = 一件饰有毒蛇小队标志性印记的重型硬甲服。其护甲在同等防护下异常灵活，并内置夜视系统。

uplink-pirate-viper-group-medic-hardsuit-name = 毒蛇小队IMP硬甲服
uplink-pirate-viper-group-medic-hardsuit-desc = 一件饰有毒蛇小队标志性印记的轻型硬甲服。其护甲在要害部位设有各种强化护板，并配备医疗HUD，辅以热脉冲系统。

uplink-pirate-webvest-name = 战术背心
uplink-pirate-webvest-desc = 一件走私进入该星区的战术背心，没有像民用承包商使用的那些被降级。穿刺抗性极佳，热抗性较差。

uplink-pirate-elite-webvest-name = 精英战术背心
uplink-pirate-elite-webvest-desc = 一件经改装、专门应对TSFMC能量武器的战术背心。热抗性极佳，穿刺抗性较差。

uplink-pirate-syndie-mask-name = 高级防毒面具
uplink-pirate-syndie-mask-desc = 一面抗闪光、可连接气源的战术防毒面具。

uplink-pirate-modsuit-name = RX-01 PDV模组服
uplink-pirate-modsuit-desc = 由乌尔曼工业开发的实验性模组服原型机。充能状态下可提供强化的敏捷性和增强的传感器。"最初客户死了所以呃，尽情玩吧" —— 费利克斯·乌尔曼

# MARK: Explosives

uplink-pirate-flashbanggrenade-name = 闪光弹
uplink-pirate-flashbanggrenade-desc = 一种能产生致盲闪光的手榴弹。

uplink-pirate-smokegrenade-name = 烟雾手榴弹
uplink-pirate-smokegrenade-desc = 一种能产生大片烟雾云的手榴弹，遮蔽其中的一切并使其躲避热成像仪。

uplink-pirate-c4-name = C-4
uplink-pirate-c4-desc = 用它炸开墙壁、处理尸体、破坏设备，或者贴在你看不顺眼的宠物身上。附带可调定时器，最短可设为10秒。

uplink-pirate-c4-bundle-name = C-4套装
uplink-pirate-c4-bundle-desc = 足量的C-4，够你炸进金库再从后门炸出来。

uplink-pirate-MobImperiroach-name = ImperiRoach
uplink-pirate-MobImperiroach-desc = 呼叫一只趁手的ImperiRoach，配备微型炸弹植入物。受重伤时会爆炸。会使用粗话和闹情绪。

uplink-pirate-empgrenade-box-name = EMP手榴弹盒
uplink-pirate-empgrenade-box-desc = 一个装有4枚EMP手榴弹的盒子。
uplink-pirate-empgrenade-name = EMP手榴弹
uplink-pirate-empgrenade-desc = 一种手持手榴弹，能释放高能脉冲，在中等偏大的范围内干扰电子设备和电力系统。

uplink-pirate-incendiarygrenade-name = 燃烧手榴弹
uplink-pirate-incendiarygrenade-desc = 一种能释放大量燃烧弹丸的手榴弹。

uplink-pirate-shrapnelgrenade-name = 破片手榴弹
uplink-pirate-shrapnelgrenade-desc = 一种能释放大量致命破片的手榴弹。

uplink-pirate-explosivegrenade-name = 爆破手榴弹
uplink-pirate-explosivegrenade-desc = 一种能引发大爆炸的手榴弹，破坏附近的基础设施和人员。

# MARK: $Gambling$

uplink-pirate-syndisurplus-name = 剩余物资箱
uplink-pirate-syndisurplus-desc = 内含大量由支持法厄同王朝的公司进口的货物。本商品不退款，别跟我们的律师争。

uplink-pirate-syndisupersurplus-name = 超级剩余物资箱
uplink-pirate-syndisupersurplus-desc = 内含巨量由支持法厄同王朝的公司进口的货物。本商品不退款，别跟我们的律师争。

# Vouchers


uplink-pdv-t0-voucher-name = PDV 0级飞船凭证
uplink-pdv-t0-voucher-desc = 一张可兑换任意小型飞船的一次性凭证。

uplink-pdv-t1-voucher-name = PDV Tier 1 Ship Voucher
uplink-pdv-t1-voucher-desc = A small card that will allow you to procure any tier-1 PDV ship from the Dynasty's reserves.

uplink-pdv-t2-voucher-name = PDV Tier 2 Ship Voucher
uplink-pdv-t2-voucher-desc = A small card that will allow you to procure any tier-2 PDV ship from the Dynasty's reserves.

uplink-pdv-t3-voucher-name = PDV Tier 3 Ship Voucher
uplink-pdv-t3-voucher-desc = A small card that will allow you to procure any tier-3 PDV ship from the Dynasty's reserves.

uplink-pdv-t1-hourglass-voucher-name = PDV Hourglass LPC [T1]
uplink-pdv-t1-hourglass-voucher-desc = 一张小卡片，内含从旗舰储备中采购Hourglass级护卫舰的数据。

uplink-pdv-t1-garm-voucher-name = PDV Garm LPC [T1]
uplink-pdv-t1-garm-voucher-desc = 一张小卡片，内含从旗舰储备中采购Garm级轰炸机型支援舰的数据。

uplink-pdv-t1-fenrir-voucher-name = PDV Fenrir LPC [T1]
uplink-pdv-t1-fenrir-voucher-desc = 一张小卡片，内含从旗舰储备中采购Fenrir级电磁炮驱逐舰的数据。

uplink-pdv-t1-kalisto-voucher-name = PDV Kalisto LPC [T1]
uplink-pdv-t1-kalisto-voucher-desc = 一张小卡片，内含从旗舰储备中采购Kalisto级火炮轻护卫舰的数据。

uplink-pdv-t1-scorpion-voucher-name = PDV Scorpion LPC [T1]
uplink-pdv-t1-scorpion-voucher-desc = 一张小卡片，内含从旗舰储备中采购Scorpion级护卫舰的数据。

uplink-pdv-t1-bastion-voucher-name = PDV Bastion LPC [T1]
uplink-pdv-t1-bastion-voucher-desc = 一张小卡片，内含从旗舰储备中采购Bastion级拳舰的数据。

uplink-pdv-t1-neptune-voucher-name = PDV Neptune LPC [T1]
uplink-pdv-t1-neptune-voucher-desc = 一张小卡片，内含从旗舰储备中采购Neptune级护卫舰的数据。

uplink-pdv-t1-layak-voucher-name = PDV Layak LPC [T1]
uplink-pdv-t1-layak-voucher-desc = 一张小卡片，内含从旗舰储备中采购Layak级轻护卫舰的数据。

uplink-pdv-t2-europa-voucher-name = PDV Europa LPC [T2]
uplink-pdv-t2-europa-voucher-desc = 一张小卡片，内含从旗舰储备中采购Europa级巡洋舰的数据。

uplink-pdv-t2-ganymede-voucher-name = PDV Ganymede LPC [T2]
uplink-pdv-t2-ganymede-voucher-desc = 一张小卡片，内含从旗舰储备中采购Ganymede级"护卫舰"的数据。

uplink-pdv-t2-vulture-voucher-name = PDV Vulture LPC [T2]
uplink-pdv-t2-vulture-voucher-desc = 一张小卡片，内含从旗舰储备中采购Vulture级采矿机甲的数据。

uplink-pdv-t2-motleyanne-voucher-name = PDV Motley Anne LPC [T2]
uplink-pdv-t2-motleyanne-voucher-desc = 一张小卡片，内含从旗舰储备中采购Motley Anne级航空母舰的数据。

uplink-pdv-t3-saintie-voucher-name = PDV Saintie LPC [T3]
uplink-pdv-t3-saintie-voucher-desc = 一张小卡片，内含从旗舰储备中采购Saintie级巡洋舰的数据。

uplink-pdv-t2-kortik-voucher-name = PDV Kortik-R LPC [T2]
uplink-pdv-t2-kortik-voucher-desc = 一张小卡片，内含从旗舰储备中采购Kortik-R级护卫舰的数据。

uplink-pdv-t3-kortik-voucher-name = PDV Kortik-EM LPC [T3]
uplink-pdv-t3-kortik-voucher-desc = 一张小卡片，内含从旗舰储备中采购Kortik-EM级护卫舰的数据。

uplink-pdv-t4-saturn-voucher-name = PDV Saturn LPC [T4]
uplink-pdv-t4-saturn-voucher-desc = 一张小卡片，授权采购Saturn。

uplink-HardBomb-PDV-name = PDV “亚当”掘地炸弹
uplink-HardBomb-PDV-desc = PDV 对不够劲爆的回应

# MARK: Materials

uplink-pdv-composite5-name = 5 坚硬金属
uplink-pdv-composite5-desc = 申请一些高级合金。

uplink-pdv-composite15-name = 15 坚硬金属
uplink-pdv-composite15-desc = 申请大量高级合金。

uplink-pdv-composite30-name = 30 坚硬金属
uplink-pdv-composite30-desc = 征用大量高级合金。
